package qzContigFilter::qzContigFilterClient;
...
qzContigFilter::qzContigFilterClient

=head1 DESCRIPTION


A KBase toy module with name: qzContigFilter
...
sub new
{
    ...
    my $self = {
	client => qzContigFilter::qzContigFilterClient::RpcClient->new,
	url => $url,
	headers => [],
    };
...
<pre>
$params is a qzContigFilter.FilterContigsParams
$return is a qzContigFilter.FilterContigsResults
FilterContigsParams is a reference to a hash where the following keys are defined:
	workspace has a value which is a qzContigFilter.workspace_name
	contigset_id has a value which is a qzContigFilter.contigset_id
...
FilterContigsResults is a reference to a hash where the following keys are defined:
	...
	new_contigset_ref has a value which is a qzContigFilter.ws_contigset_id
...
# Authentication: required
	...
	method => "qzContigFilter.filter_contigs",
	params => \@args,
    });
    if ($result) {
	if ($result->is_error) {
...
sub _validate_version {
    ...
    if ($sMinor > $cMinor) {
        warn "New client version available for qzContigFilter::qzContigFilterClient\n";
    }
    if ($sMajor == 0) {
        warn "qzContigFilter::qzContigFilterClient version is $svr_version. API subject to change.\n";
    }
}
...
package qzContigFilter::qzContigFilterClient::RpcClient;
